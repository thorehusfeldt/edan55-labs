...
With $10\,000$ runs algorithm Monte-Carlo, which takes $[\ldots]$ seconds to execute, is within $[\ldots]$ digits\sidenote{Here we are after the relative accuracy, which we define as the number of leading zeroes in the expression $\frac{E[G] - Avg(MonteCarlo(G))}{E[G]}$, where $E[G]$ is the estimated delivery time.} of accuracy on the input toy.txt.\sidenote{Take the minimum of FedUPS and PostNHL compared to the output file.}


\subsection{Solution quality Small}

$[\ldots]$ runs, and $[\ldots]$ seconds are needed for the Monte-Carlo algorithm on input small.txt to be within $3$ digits of accuracy for 10 consecutive runs.\sidenote{
...
\end{tabular}\sidenote{If you encounter problems with strange1.txt, draw the graph.}
