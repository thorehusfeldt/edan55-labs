...
\title{Approximation Algorithm for Maximum Cut}
\begin{document}
\maketitle
...
Maxcut is NP-hard, which is usually shown by an easy reduction from
Not-All-Equal 3-Satisfiability.
Thus, we have little hope of writing an algorithm that solves
arbitrary Maxcut instances optimally.
...
Then, $\Pr(X_{uv} = 1) = [\ldots]$.\sidenote{Determine this
  probability.
  Your calculation needs to include a phrase like ``because BLA and
  BLA are independent'' somewhere.}
Now, $E[C]=[\ldots]$ \sidenote{Continue the argument until you
  establish \eqref{eq: E[C]}.
  A phrase like ``by linearity of expectation'' must appear somewhere.}
...
``bad instance'' and a comparison to a hyphothetical optimum,
respectively).
...
The reduction from NAE-Sat (to establish that Maxcut is NP-hard) can
be found in a number of places, such as XXX.
...
The \emph{determinisitc} algorithm of Sahni and Gonzales\sidenote{S.\
